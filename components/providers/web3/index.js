...
export default function Web3Provider({children}) {
  ...
  //Functionality for Connect button. useMemo used for caching purposes. 
  const _web3Api = useMemo(() => {
    const { web3, provider, isLoading } = web3Api
    return {
      ...web3Api,
      requireInstall: !isLoading && !web3 , //If metamask is there or not. accordingly the buttong will be displayed
      connect: provider ?
        async () => {
          try {
